...
class Destination implements Post {
	...
	public static function add( Entity\Destination $details ): ?int {
		...
		if ( is_wp_error( $post_id ) ) {
			Logger::error( "[{$details->getId()}] Destination error: {$post_id->get_error_message()}" );

			return null;
		}

		if ( $originalPost->post_count === 1 ) {
			Logger::modify( "[{$post_details['meta_input'][ Feed\Destination::$metaKeyId ]}] Updated {$post_details['post_type']} post {$originalPost->post->post_title}" );
		} else {
